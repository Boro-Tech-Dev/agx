-- ColBERT-only stack: enable catalog rows, disable TEI sidecars, repoint tool-capable agents.
-- Idempotent — safe to run on every deploy via scripts/apply-retrieval-seeds.sh.
...
ON CONFLICT (reranker_id) DO UPDATE SET
...
UPDATE reranker_catalog SET enabled = false
WHERE reranker_id IN ('tei_bge', 'tei_jina');
...
ON CONFLICT (agent) DO UPDATE SET
  ...
  updated_at = now()
WHERE agent_retrieval_config.agent IN ('pm', 'builder', 'forge', 'canon')
   OR agent_retrieval_config.reranker_id IN ('tei_bge', 'tei_jina');
