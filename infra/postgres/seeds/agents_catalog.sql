-- Re-upsert the built-in agents (same rows as schema.sql bootstrap).
-- Fresh Compose DBs load agents from infra/postgres/schema.sql automatically.
-- Apply this manually when updating catalog SQL against an existing database:
--   psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f infra/postgres/seeds/agents_catalog.sql
