"""Canonical editorial / PRB workflow phases for timeline normalization (catalog_version bumps on list changes)."""
...
SCHEDULE_CONSTRAINTS = (
    'For PRB1 we submit layouts but need to submit coded (HTML) assets or mechanicalized files for PRB2. '
    'All production is started in parallel with PRB1.'
)

# Ordered phases — duplicate human labels (e.g. "Layout Revisions") use unique phase_id.
PHASE_ROWS: list[PhaseDef] = [
...
def phase_by_id() -> dict[str, PhaseDef]:
    return {p['phase_id']: p for p in PHASE_ROWS}


def catalog_prompt_block() -> str:
    lines = [f"{p['order']}. {p['phase_id']} — {p['label']}" for p in PHASE_ROWS]
    return 'Canonical phases (in order):\n' + '\n'.join(lines) + '\n\nConstraints:\n' + SCHEDULE_CONSTRAINTS
