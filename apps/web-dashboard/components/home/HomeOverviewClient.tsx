...
import { HomeOverviewSkeleton } from './HomeOverviewSkeleton';
import { HomeSystemStrip } from './HomeSystemStrip';

const DashboardShell = dynamic(
  () => import('../DashboardShell').then((m) => ({ default: m.DashboardShell })),
  { ssr: false },
);

const HomeHeaderClient = dynamic(
  () => import('./HomeHeaderClient').then((m) => ({ default: m.HomeHeaderClient })),
  { ssr: false },
);

const HomeHero = dynamic(() => import('./HomeHero').then((m) => ({ default: m.HomeHero })), {
  ssr: false,
});

const HomeDashboardHub = dynamic(
  () => import('./HomeDashboardHub').then((m) => ({ default: m.HomeDashboardHub })),
  { ssr: false },
);
...
type HomeOverviewData = {
  ...
  cmdkAgents: { key: string; name: string }[];
  monitoring: Awaited<ReturnType<typeof getQueueMonitoring>> | null;
  monitoringError: string | null;
  strip: HomeOverviewStrip;
  initialProjects: unknown[];
};

async function loadHomeOverviewData(): Promise<HomeOverviewData> {
  ...
  const runsError =
    runsResult.status === 'rejected' ? String((runsResult.reason as Error)?.message || runsResult.reason) : null;
  ...
  const agentsError =
    agentsResult.status === 'rejected'
      ? String((agentsResult.reason as Error)?.message || agentsResult.reason)
      : null;
  ...
  return {
    ...
    runsError,
    agentsError,
    cmdkAgents: agents.map(({ key, name }) => ({ key, name })),
    monitoring,
    monitoringError,
    strip,
    initialProjects: Array.isArray(projectsRows) ? (projectsRows as unknown[]) : [],
  };
}
...
export function HomeOverviewClient() {
    ...
    loadHomeOverviewData()
      ...
      .catch(() => {
        if (!cancelled) {
          setData({
            runs: [],
            runsError: 'Failed to load dashboard data',
            agentsError: null,
            cmdkAgents: [],
            monitoring: null,
            monitoringError: 'Failed to load dashboard data',
            strip: {
              monitoring: null,
              monitoringError: 'Failed to load dashboard data',
              projectsCount: null,
              projectsError: 'Failed to load dashboard data',
              workspacesCount: null,
              workspacesError: 'Failed to load dashboard data',
              pendingApprovals: null,
              approvalsTotal: null,
              approvalsError: 'Failed to load dashboard data',
              tokensConsumed: null,
  ...
  const { runs, runsError, agentsError, cmdkAgents, monitoring, monitoringError, strip, initialProjects } =
    data;
