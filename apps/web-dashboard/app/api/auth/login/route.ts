...
export async function POST(req: NextRequest) {
  if (isAuthDisabled()) {
    return NextResponse.json({ error: 'Authentication is disabled (AUTH_DISABLED).' }, { status: 400 });
  }
  ...
  if (!keycloakBaseUrl()) {
    return NextResponse.json(
      { error: 'Sign-in is not configured (set KEYCLOAK_BASE_URL on the server, e.g. http://keycloak:8080 in Docker).' },
      { status: 503 },
    );
  }
  ...
  if (grant.ok === false) {
    ...
    if (status === 503) {
      return NextResponse.json({ error: grantErrorMessage(bodyText) }, { status: 503 });
    }
    if (status >= 500) {
      return NextResponse.json(
        { error: `Keycloak is unreachable or returned an error (${status}). ${grantErrorMessage(bodyText)}` },
        { status: 503 },
      );
    }
    return NextResponse.json({ error: grantErrorMessage(bodyText) }, { status: 401 });
  }
