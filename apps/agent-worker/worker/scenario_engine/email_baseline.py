"""Reference schedule for HCP MLR / generic-tactic tests — parity with emailBaseline.ts."""
...
class EmailBaselineRow(TypedDict):
    phase_id: str
    start_date: str
    end_date: str
    note: str
...
EMAIL_BASELINE_KICKOFF_ISO = '2026-03-02'

EMAIL_BASELINE: list[EmailBaselineRow] = [
    {
        'phase_id': 'kickoff',
    ...
    },
    {
        ...
        'end_date': '2026-04-27',
        'note': (
            'Address PRB1 feedback. Production begins in parallel with PRB1 and continues toward '
            'coded HTML/mechanicalized assets for PRB2.'
        ),
    },
    {
...
def _assert_baseline_matches_catalog() -> None:
    if len(EMAIL_BASELINE) != len(PHASE_CATALOG):
        raise RuntimeError(
            f'EMAIL_BASELINE length {len(EMAIL_BASELINE)} !== PHASE_CATALOG {len(PHASE_CATALOG)}'
        )
    for i, (eb, pc) in enumerate(zip(EMAIL_BASELINE, PHASE_CATALOG, strict=True)):
        if eb['phase_id'] != pc['phase_id']:
            raise RuntimeError(
                f'EMAIL_BASELINE phase_id mismatch at {i}: {eb["phase_id"]} vs {pc["phase_id"]}'
            )


_assert_baseline_matches_catalog()

BASELINE_BY_ID: dict[str, EmailBaselineRow] = {r['phase_id']: r for r in EMAIL_BASELINE}
